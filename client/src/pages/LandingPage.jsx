    ...
    <div className="min-h-screen bg-[#0f0f23] text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 glass border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link to="/register" className="gradient-bg px-5 py-2 rounded-xl text-sm font-semibold hover:brightness-110 transition-all shadow-lg shadow-indigo-500/25">Get Started</Link>
          </div>
        </div>
      </nav>
      ...
      <section className="relative pt-32 pb-20 px-6">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          <div className="absolute top-40 right-1/4 w-80 h-80 bg-violet-600/20 rounded-full blur-[100px] animate-pulse" style={{ animationDelay: '1s' }} />
          <div className="absolute bottom-20 left-1/2 w-72 h-72 bg-purple-600/15 rounded-full blur-[100px] animate-pulse" style={{ animationDelay: '2s' }} />
        </div>
        <div className="max-w-5xl mx-auto text-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-indigo-500/30 bg-indigo-500/10 text-indigo-300 text-sm mb-8">
              <HiOutlineSparkles className="text-indigo-400" /> AI-Powered Analytics Platform
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.6 }} className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/register" className="gradient-bg px-8 py-3.5 rounded-xl font-semibold text-lg hover:brightness-110 transition-all shadow-xl shadow-indigo-500/30 flex items-center justify-center gap-2">
            ...
            {features.map((f, i) => (
              <AnimatedSection key={f.title}>
                <motion.div variants={fadeUp} custom={i} className="glass rounded-2xl p-6 card-hover group cursor-default h-full">
                  <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-indigo-500/20 to-violet-500/20 flex items-center justify-center mb-4 group-hover:from-indigo-500/30 group-hover:to-violet-500/30 transition-all">
                    <f.icon className="text-2xl text-indigo-400" />
                  </div>
      ...
      <section className="py-24 px-6 bg-gradient-to-b from-transparent via-indigo-950/10 to-transparent">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            <div className="hidden md:block absolute top-16 left-[20%] right-[20%] h-0.5 bg-gradient-to-r from-indigo-500/50 via-violet-500/50 to-indigo-500/50" />
            {steps.map((s, i) => (
              <AnimatedSection key={s.title}>
                <motion.div variants={fadeUp} custom={i} className="text-center relative">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center mx-auto mb-6 shadow-lg shadow-indigo-500/30 relative z-10">
        ...
        <AnimatedSection>
          <motion.div variants={fadeUp} className="max-w-4xl mx-auto gradient-bg rounded-3xl p-12 md:p-16 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-indigo-100 mb-8 max-w-xl mx-auto">Start analyzing your datasets with AI-powered insights. No credit card required.</p>
              <Link to="/register" className="inline-flex items-center gap-2 bg-white text-indigo-700 px-8 py-3.5 rounded-xl font-semibold text-lg hover:bg-gray-100 transition-all shadow-xl">
                Get Started Free <HiArrowRight />
              </Link>
            </div>
          </motion.div>
